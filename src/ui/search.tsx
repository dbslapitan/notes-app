...
export default function Search({className = ""}: {className?: string}) {
  ...
  return(
    <div className={`relative mt-4 ${className}`}>
      ...
      <input type="text" defaultValue={searchParams.get("search") || ""} className={`${text["preset-5"]} w-full p-4 pl-11 leading-[1.3] border border-neutral-300 rounded-[0.5rem]`} onChange={(e) => handleInputChange(e.target.value)}/>
    </div>
  );
}